...
function getPartsMap()
{
	...
	$partsMap = array();
	$path = JPATH_INSTALLATION.DS.'sql';
	$totalSize = 0;
	$runSize = 0;
	...
	for($index = 0; $index <= $parts; $index++)
	{
		...
		$filesize = @filesize($path.DS.$basename) ;
		$totalSize += intval($filesize);
		$partsMap[] = $path.DS.$basename;
	}
...
function openFile()
{
	...
	if ( !$file = @fopen($filename, "rt") ) {
		throwError(ABIText::_('ERROR_CANTOPENDUMPFILE').' '.$sqlfile);
		return false;
	}
	else
	{
...
while (($linenumber < ($start + LINESPERSESSION)) && ($totalsizeread <= BYTESPERSESSION)) {
