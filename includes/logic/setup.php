...
// Is this Joomla! 1.6?
$joomla15 = @file_exists( JPATH_SITE.DS.'includes'.DS.'joomla.php' );
if($joomla15) {
	$sql = 'SELECT `id`, `username`, `email` FROM `#__users` WHERE `usertype`="Super Administrator"';
} else {
	$sql = 'SELECT `id`, `username`, `email` FROM `#__users` AS u INNER JOIN `#__user_usergroup_map` AS m ON m.`user_id` = u.`id` '.
		'WHERE m.group_id = 8';
}
