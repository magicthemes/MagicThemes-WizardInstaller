...
{
	$dbini_rel = 'sql'.DS.'databases.ini';
	$dbini_abs = JPATH_INSTALLATION.DS.$dbini_rel;
	...
	if(!file_exists($databasesIniFilename) || !is_readable($databasesIniFilename))
	{
...
if(!isset($view['maxchunk'])) $view['maxchunk'] = 1048756;
if(!isset($view['maxqueries'])) $view['maxqueries'] = 1000;
...
if($automation->hasAutomation())
{
	...
	if( isset($databases[$activeDatabase]['maxchunk']) )
	{
		$view['maxchunk'] = $databases[$activeDatabase]['maxchunk'];
	}

	if( isset($databases[$activeDatabase]['maxqueries']) )
	{
		$view['maxqueries'] = $databases[$activeDatabase]['maxqueries'];
	}
}
